...
import com.prio.pariwisata.databinding.LayoutRegisterBinding;
...
public class Register extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        binding.goLogin.setOnClickListener(v-> startActivity(new Intent(this,Login.class)));
        binding.btnRegister.setOnClickListener(v-> {
            if (Objects.requireNonNull(binding.username.getText()).toString().isEmpty() ||
                ...
                Objects.requireNonNull(binding.email.getText()).toString().isEmpty()
                )
    ...
    private void RequestRegister(String username,String email,String password){
        Call<Calling_User> calling_user = client.UserRegister(username,password,email);
        calling_user.enqueue(new Callback<Calling_User>() {
            ...
            public void onResponse(@NonNull Call<Calling_User> call, @NonNull Response<Calling_User> response) {
                ...
                if (Calling_User.TreatResponse(Register.this,"register",response)){
                    Log.i(TAG, "onResponse: oke in " + Model_User.sg.token);
                    assert callingUser != null;
                    Model_User.sg.token = callingUser.data.token;
                    startActivity(new Intent(Register.this, ActivityParent.class));
                    finish();
                }
            }
